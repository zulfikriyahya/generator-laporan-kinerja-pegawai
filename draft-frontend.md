  ...
  integrations: [
    ...
    AstroPWA({
      ...
      manifest: {
        ...
        description: 'Aplikasi penyusun laporan kinerja pegawai otomatis berbasis AI',
        theme_color: '#0f172a',
...
<div class="space-y-4 animate-fade-in">
	...
	>
		ℹ️ Khusus untuk Pegawai dengan Jabatan Guru (fungsional).
	</div>
	...
	<div class="grid grid-cols-2 gap-3">
		<InputGroup
			...
			name="tapel"
			model="form.akademik.tahunPelajaran"
	...
	<SelectGroup
		...
		name="mapel"
		model="form.akademik.mapel"
		options={[
			{ val: "Matematika", label: "Matematika" },
			{ val: "Bahasa Indonesia", label: "Bahasa Indonesia" },
			{ val: "Bahasa Inggris", label: "Bahasa Inggris" },
			{ val: "IPA (Ilmu Pengetahuan Alam)", label: "IPA" },
			{ val: "IPS (Ilmu Pengetahuan Sosial)", label: "IPS" },
			{ val: "Pendidikan Agama Islam", label: "Pendidikan Agama Islam" },
			{ val: "Pendidikan Agama Kristen", label: "Pendidikan Agama Kristen" },
			{ val: "Pendidikan Agama Katolik", label: "Pendidikan Agama Katolik" },
			{ val: "Pendidikan Agama Hindu", label: "Pendidikan Agama Hindu" },
			{ val: "Pendidikan Agama Buddha", label: "Pendidikan Agama Buddha" },
			{ val: "PJOK (Pendidikan Jasmani)", label: "PJOK" },
			{ val: "Seni Budaya", label: "Seni Budaya" },
			{ val: "Prakarya", label: "Prakarya" },
			{ val: "Informatika", label: "Informatika" },
			{ val: "Bahasa Arab", label: "Bahasa Arab" },
			{ val: "Al-Quran Hadits", label: "Al-Quran Hadits" },
			{ val: "Akidah Akhlak", label: "Akidah Akhlak" },
			{ val: "Fiqih", label: "Fiqih" },
			{ val: "Sejarah Kebudayaan Islam", label: "Sejarah Kebudayaan Islam" },
		]}
	/>
	...
	<InputGroup
		...
		name="kls"
		model="form.akademik.kelas"
		placeholder="VII-A, VII-B, VIII-A"
	/>
	...
	<div class="grid grid-cols-2 gap-3">
		...
		<InputGroup
			label="Jumlah Siswa"
			name="siswa"
			model="form.akademik.jumlahSiswa"
			type="number"
			placeholder="64"
		/>
	</div>
	...
	<InputGroup
		label="Tugas Ekstrakurikuler (Opsional)"
		name="ekskul"
		model="form.akademik.ekskul"
		placeholder="Olimpiade Matematika, Pramuka"
	/>
</div>
...
<div class="space-y-4 animate-fade-in pb-10">
	...
	<div class="grid grid-cols-2 gap-3">
		<SelectGroup
			...
			options={[
				...
				{ val: "PTT", label: "PTT (Pegawai Tidak Tetap)" },
				{ val: "Guru", label: "Guru" },
			]}
		/>
		<InputGroup
	...
	<SelectGroup
		label="Jabatan"
		name="jab"
		model="form.pegawai.jabatan"
		options={[
			{ val: "Guru Ahli Pertama", label: "Guru Ahli Pertama" },
			{ val: "Guru Ahli Muda", label: "Guru Ahli Muda" },
			{ val: "Guru Ahli Madya", label: "Guru Ahli Madya" },
			{ val: "Guru Ahli Utama", label: "Guru Ahli Utama" },
			{ val: "Kepala Sekolah", label: "Kepala Sekolah" },
			{ val: "Wakil Kepala Sekolah", label: "Wakil Kepala Sekolah" },
			{ val: "Kepala Tata Usaha", label: "Kepala Tata Usaha" },
			{ val: "Bendahara", label: "Bendahara" },
			{ val: "Operator", label: "Operator" },
			{ val: "Pustakawan", label: "Pustakawan" },
			{ val: "Laboran", label: "Laboran" },
			{ val: "Teknisi", label: "Teknisi" },
			{ val: "Tenaga Administrasi", label: "Tenaga Administrasi" },
			{ val: "Penjaga Sekolah", label: "Penjaga Sekolah" },
			{ val: "Tenaga Kebersihan", label: "Tenaga Kebersihan" },
		]}
	/>
...
export const reportStore = persistentMap<AppStore>('ekinerja-app-v1:', defaultState, {
...
export const historyStore = persistentMap<HistoryStore>('ekinerja-history:', defaultHistoryState, {
  encode: JSON.stringify,
  decode: JSON.parse,
});
...
export const updateStore = <K extends keyof AppStore>(key: K, value: AppStore[K]) => {
...
export const validatePegawai = (data: AppStore): string[] => {
  ...
  if (data.pegawai.jenis === 'Guru') {
    if (!data.akademik.mapel) errors.push('Mata pelajaran harus diisi untuk Guru');
...
export const ValidationRules = {
  ...
  phone: {
    ...
    message: 'Nomor telepon tidak valid (format: 08xxxxxxxxxx atau +628xxxxxxxxxx)',
  },
...
export const validateMarkdown = (markdown: string): { valid: boolean; warnings: string[] } => {
...
export const importTTEData = async (file: File): Promise<TTEData | null> => {
  try {
    ...
    if (!data.qrCode || !data.nomorDokumen || !data.hashDokumen || !data.timestamp) {
 ...
 */
export const addWatermarkToCanvas = (canvas: HTMLCanvasElement, text: string): void => {
 ...
 */
export const logTTEGeneration = (tte: TTEData, pegawai: { nama: string; nip: string }): void => {
  const auditLog = {
  ...
  kinerja: [
    {
      ...
      'Target Tahunan (IKU)': 'Meningkatkan nilai rata-rata kelas di atas KKM (75)',
 ...
 */
const mergeImportedData = (current: AppStore, imported: Partial<AppStore>): AppStore => {
  return {
    ...current,
    ...(imported.instansi && {
 ...
 */
export const uploadToGoogleDrive = async (blob: Blob): Promise<ExportResult> => {
  // Placeholder untuk future implementation
  return {
    success: false,
...
import { reportStore, updateStore, generateNomorDokumen } from '../stores/reportStore';
...
const buildUserPrompt = (store: AppStore): string => {
  ...
  const namaBulan = new Date(parseInt(config.tahun), parseInt(config.bulan) - 1).toLocaleString(
    'id-ID',
    { month: 'long' },
  );
...
export const generateLaporan = async (model?: AIModel): Promise<GenerateAIResult> => {
  const store = reportStore.get();
